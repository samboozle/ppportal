...
const promptFuncs = {
    writeYesteryearChronicles,
    rewriteSegment
};

export const POST = async ({ request }) => {
    const { func, details } = await request.json();
    let written;
    switch (func) {
        case "writeYesteryearChronicles":
            written = await writeYesteryearChronicles(details.episode);
            return json(written);
        case "rewriteYesteryearChronicles":
            written = await rewriteSegment();
    }
    return json(written);
};
