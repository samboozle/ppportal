...
socket.on("updateYesteryearEpisode", (message) => {
  yesteryearEpisode.update((episode) => {
    episode[message.segment as YesteryearSegment] = message.lines.map((line: any) => ({
      ...line,
      recording: []
    }));
    return episode;
...
socket.on("voiceLineChunkGenerated", (message) => {
  const chunk = new Uint8Array(message.chunk);
  yesteryearEpisode.update((episode) => {
    episode[message.segment as YesteryearSegment][message.index].recording = [
      ...episode[message.segment as YesteryearSegment][message.index].recording,
      chunk
    ];
    return episode;
